...
test("check tgd head is parsed correctly", () => {
  ...
  expect(myParser.tgds[0].head.predicate.variables.length).toBe(1);
  expect(myParser.tgds[0].head.predicate.variables[0].name).toBe("x");
})
...
test("check tgd body first predicate is parsed correctly", () => {
  ...
  expect(myParser.tgds[0].body.predicates[0].variables.length).toBe(2);
  expect(myParser.tgds[0].body.predicates[0].variables[0].name).toBe("x");
  expect(myParser.tgds[0].body.predicates[0].variables[1].name).toBe("y");
  
})

test("check tgd body second predicate is parsed correctly", () => {
  expect(myParser.tgds[0].body.predicates[0].name).toBe("r2");
  expect(myParser.tgds[0].body.predicates[0].variables.length).toBe(1);
  expect(myParser.tgds[0].body.predicates[0].variables[0].name).toBe("y");s
})

...
public class RuleTest extends TestCase {
	...
	private Set<ILiteral> HEAD;

	private Set<ILiteral> BODY;
	...
	@Override
	public void setUp() {
		HEAD = Sets.newHashSet();
		ILiteral literal = BASIC.createLiteral(true,
		        BASIC.createAtom(BASIC.createPredicate("sin", 1), BASIC.createTuple(CONCRETE.createInteger(1))));
		HEAD.add(literal);
		...
		literal = BASIC.createLiteral(
		        ...
		        BASIC.createAtom(
		                ...
		                BASIC.createTuple(CONCRETE.createInteger(2005), CONCRETE.createInteger(12),
		                        CONCRETE.createInteger(24))));
		HEAD.add(literal);

		BODY = Sets.newHashSet();
	...
	public void testEquals() {
		ObjectTests.runTestEquals(new Rule(HEAD, BODY), new Rule(HEAD, BODY), new Rule(HEAD, HEAD));
		ObjectTests.runTestEquals(new Rule(HEAD, BODY), new Rule(HEAD, BODY), new Rule(BODY, BODY));
	}

...
public class MagicSetsTest extends TestCase {
	...
	return createAdornedLiteral(MagicSets.MAGIC_PREFIX + "_" + symbol, ad,
		t);
    }
    ...
    private static ILiteral createLabeledLiteral(final String symbol,
	    final Adornment[] ad, final ITerm[] t) {
	return createAdornedLiteral(MagicSets.LABEL_PREFIX + "_" + symbol, ad,
		t);
    }
     ...
     * 
     * @param positive
     *            <code>true</code> whether the resulting literal should be
     *            positive, or not.
     * @param symbol
     *            the predicate symbot to use for the literal
     * @param ad
     ...
     * @return the constructed magic literal
     */
    private static ILiteral createAdornedLiteral(final boolean positive,
	    final String symbol, final Adornment[] ad, final ITerm[] t) {
	...
	return BASIC.createLiteral(positive, BASIC.createPredicate(newSymbol
		.toString(), t.length), BASIC.createTuple(t));
    }
     ...
     * 
     * @param a
     *            the atom for which to create the seed rule
	...
	return BASIC.createRule(Arrays.asList(l), Collections
		.<ILiteral> emptyList());
    }

    /**
     * Parses a program and returns the result of the magic sets transformation.
     * 
     * @param s
     ...
     * 
     * @param expected
     *            the expected transformation result
     * @param result
     *            the real transformation result
     */
	...
	if (expected == null) { // test the failed transformation
	    ...
	    assertEquals("The rules are computed as expected", expected.rules,
		    result.rules);
	    assertEquals("The query is not correct", expected.query,
		    result.query);
	}
    }
	...
	final Adornment[] BF = new Adornment[] { Adornment.BOUND,
		Adornment.FREE };
	...
	ref.add(seedRule(createMagicLiteral("sg", BF, new ITerm[] { TERM
		.createString("john") })));

	// constructing the magic rules

	// magic_sg_bf(Z1) :- magic_sg_bf(X), up(X, Z1)
	List<ILiteral> head = Arrays.asList(createMagicLiteral("sg", BF,
		new ITerm[] { Z1 }));
	List<ILiteral> body = Arrays.asList(createMagicLiteral("sg", BF,
		new ITerm[] { X }), createLiteral("up", "X", "Z1"));
	ref.add(BASIC.createRule(head, body));
	// magic_sg_bf(Z3) :- magic_sg_bf(X), up(X, Z1), sg(Z1, Z2), flat(Z2,
	// Z3)
	...
	ref.add(BASIC.createRule(head, body));
	// sg(X,Y) :- magic_sg_bf(X), up(X, Z1), sg(Z1, Z2), flat(Z2, Z3),
	// sg(Z3, Z4), down(Z4, Y)
	head = Arrays.asList(createLiteral("sg", "X", "Y"));
	body = Arrays.asList(createMagicLiteral("sg", BF, new ITerm[] { X }),
		createLiteral("up", "X", "Z1"),
	...
	final Adornment[] bbf = new Adornment[] { Adornment.BOUND,
		Adornment.BOUND, Adornment.FREE };
	final Adornment[] bff = new Adornment[] { Adornment.BOUND,
		Adornment.FREE, Adornment.FREE };
	...
	ref.add(seedRule(createMagicLiteral("a", bbf, new ITerm[] {
		TERM.createString("john"), TERM.createString("mary") })));

	// constructing the magic/labeled rules rules

	// label_a_1_bbf(X, A) :- magic_a_bbf(X, Y), b(X, A)
	List<ILiteral> head = Arrays
		.asList(createLabeledLiteral("a_1", bbf, XA));
	List<ILiteral> body = Arrays.asList(createMagicLiteral("a", bbf, XY),
		createLiteral("b", "X", "A"));
	ref.add(BASIC.createRule(head, body));
	// label_a_2_bff(X) :- magic_a_bbf(X, Y)
	head = Arrays
		.asList(createLabeledLiteral("a_2", bff, new ITerm[] { X }));
	...
	body = Arrays.asList(createMagicLiteral("a", bbf, XY), createLiteral(
		"c", "X", "Y", "Z"));
	ref.add(BASIC.createRule(head, body));
	// a(X, Y, Z) :- magic_a_bbf(X, Y), b(X, A), a(X, A, B), c(B, Y, Z)
	head = Arrays.asList(createLiteral("a", "X", "Y", "Z"));
	body = Arrays.asList(createMagicLiteral("a", bbf, XY), createLiteral(
		"b", "X", "A"), createLiteral("a", "X", "A", "B"),
	...
	final Adornment[] bf = new Adornment[] { Adornment.BOUND,
		Adornment.FREE };
	final ILiteral b = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("b", 2), BASIC.createTuple(X, Z)));
	final ILiteral c = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("c", 3), BASIC.createTuple(TERM
		.createString("a"), Z, Y)));
	final ILiteral x = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("x", 3), BASIC.createTuple(X, Y, Z)));

	final List<IRule> ref = new ArrayList<IRule>();
	// magic_c_bbf(a, Z) :- magic_a_bf(X), b(X, Z)
	ref.add(BASIC.createRule(Arrays.asList(createMagicLiteral("c", bbf,
		new ITerm[] { a, Z })), Arrays.asList(createMagicLiteral("a",
		bf, new ITerm[] { X }), b)));
	...
	ref.add(BASIC.createRule(Arrays
		.asList(createLiteral("c", "X", "Y", "Z")), Arrays.asList(
		createMagicLiteral("c", bbf, new ITerm[] { X, Y }), x)));
	// magic_a_bf('john') :- .
	ref.add(seedRule(createMagicLiteral("a", bf, new ITerm[] { TERM
		.createString("john") })));
    ...
    /**
     * Tests whether useless magic predicates (magic_q^f()) will be created, or
     * not.
     */
     ...
     * 
     * @see <a
     *      href="http://sourceforge.net/tracker/index.php?func=detail&aid=1798276&group_id=167309&atid=842434">bug
     *      #1798276: Magic Sets evaluation does not allow conjunctive
     *      queries</a>
     */
	...
	final ITerm[] XYZ = new ITerm[] { X, Y, Z };
	final Adornment[] fb = new Adornment[] { Adornment.FREE,
		Adornment.BOUND };
	final Adornment[] bb = new Adornment[] { Adornment.BOUND,
		Adornment.BOUND };
	final Adornment[] bbf = new Adornment[] { Adornment.BOUND,
		Adornment.BOUND, Adornment.FREE };
	final ILiteral c2 = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("c", 2), BASIC.createTuple(XY)));
	final ILiteral c3 = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("c", 3), BASIC.createTuple(XYZ)));
	final ILiteral p2 = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("p", 2), BASIC.createTuple(X, TERM
		.createString("a"))));
	final ILiteral r3 = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("r", 3), BASIC.createTuple(TERM
		.createString("b"), X, Y)));

	final List<IRule> ref = new ArrayList<IRule>();
	// magic_r_bbf(b, X) :- p(X, a)
	ref
		.add(BASIC.createRule(Arrays.asList(createMagicLiteral("r",
			bbf, new ITerm[] { TERM.createString("b"), X })),
			Arrays.asList(p2)));
	// magic_s_bb(e, Y) :- p(X, a), r(b, X, Y)
	ref.add(BASIC.createRule(Arrays.asList(createMagicLiteral("s", bb,
		new ITerm[] { TERM.createString("e"), Y })), Arrays.asList(p2,
		r3)));
	// p(X, Y) :- magic_p_fb(Y), c(X, Y)
	ref.add(BASIC.createRule(Arrays.asList(createLiteral("p", "X", "Y")),
		Arrays.asList(createMagicLiteral("p", fb, new ITerm[] { Y }),
			c2)));
	// r(X, Y, Z) :- magic_r_bbf(X, Y), c(X, Y, Z)
	ref.add(BASIC.createRule(Arrays
		.asList(createLiteral("r", "X", "Y", "Z")), Arrays.asList(
		createMagicLiteral("r", bbf, XY), c3)));
	...
	// p_fb('a') :- .
	ref.add(seedRule(createMagicLiteral("p", fb, new ITerm[] { TERM
		.createString("a") })));
	...
	ref.add(BASIC.createRule(Arrays.asList(createMagicLiteral("r", bbf,
		new ITerm[] { TERM.createString("b"), X })), Arrays
		.asList(createLiteral("p", "X", "Y"))));
	// magic_s_bb(e, Z) :- p(X, Y), r(b, X, Z)
	ref.add(BASIC.createRule(Arrays.asList(createMagicLiteral("s", bb,
		new ITerm[] { TERM.createString("e"), Z })), Arrays.asList(
		createLiteral("p", "X", "Y"), r3)));
	// r(X, Y, Z) :- magic_r_bbf(X, Y), c(X, Y, Z)
	ref.add(BASIC.createRule(Arrays
		.asList(createLiteral("r", "X", "Y", "Z")), Arrays.asList(
		createMagicLiteral("r", bbf, XY), c3)));
	...
	final ILiteral c3 = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("c", 3), BASIC.createTuple(XYZ)));
	final ILiteral p2 = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("p", 2), BASIC.createTuple(TERM
		.createString("b"), TERM.createString("a"))));
	final ILiteral r3 = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("r", 3), BASIC.createTuple(TERM
		.createString("b"), X, Y)));
	...
	ref.add(BASIC.createRule(Arrays.asList(createMagicLiteral("r", bff,
		new ITerm[] { TERM.createString("b") })), Arrays.asList(p2)));
	...
	ref.add(BASIC.createRule(Arrays
		.asList(createLiteral("r", "X", "Y", "Z")), Arrays.asList(
		createMagicLiteral("r", bff, new ITerm[] { X }), c3)));
	// s(X, Y) :- magic_s_bb(X, Y), c(X, Y)
	ref.add(BASIC.createRule(Arrays.asList(createLiteral("s", "X", "Y")),
		Arrays.asList(createMagicLiteral("s", bb, XY), c2)));
	// p^ff() :- TRUE
	ref.add(seedRule(createMagicLiteral("p", bb, new ITerm[] {
		TERM.createString("b"), TERM.createString("a") })));
	...
	final Adornment[] bb = new Adornment[] { Adornment.BOUND,
		Adornment.BOUND };
	final Adornment[] fbb = new Adornment[] { Adornment.FREE,
	...
	ref.add(BASIC.createRule(Arrays
		.asList(createMagicLiteral("r", fbb, YX)), Arrays.asList(c2)));
	// magic_s_bb(e, Z) :- p(X, Y), r(b, X, Z)
	ref.add(BASIC.createRule(Arrays.asList(createMagicLiteral("s", bb,
		new ITerm[] { TERM.createString("e"), Z })), Arrays.asList(
		createLiteral("p", "X", "Y"), r3)));
	...
	ref.add(BASIC.createRule(Arrays
		.asList(createLiteral("r", "X", "Y", "Z")), Arrays.asList(
		createMagicLiteral("r", fbb, YZ), c3)));
	// r(X, Y, Z) :- magic_r_bbf(X, Y), c(X, Y, Z)
	ref.add(BASIC.createRule(Arrays
		.asList(createLiteral("r", "X", "Y", "Z")), Arrays.asList(
		createMagicLiteral("r", bbf, XY), c3)));
     ...
     * 
     * @see <a
     *      href="http://sourceforge.net/tracker/index.php?func=detail&aid=1829204&group_id=167309&atid=842434">bug
     *      #1829204: Repeated literal in query fails with magic sets</a>
     */
	...
	final ILiteral magic_p = createMagicLiteral("p", b,
		new ITerm[] { CONCRETE.createInteger(1) });
	final ILiteral p1 = BASIC.createLiteral(true, BASIC.createAtom(BASIC
		.createPredicate("p", 1), BASIC.createTuple(CONCRETE
		.createInteger(1))));

	final List<IRule> ref = new ArrayList<IRule>();
	// magic_p^b(1) :- .
	ref.add(BASIC.createRule(Arrays.asList(magic_p), Collections
		.<ILiteral> emptyList()));
	// magic_p^b(1) :- p(1).
    ...
    /**
     * Test to ensure the correct handling of conjunctive queries with negative
     * literals.
     * 
     * @see <a
     *      href="https://sourceforge.net/tracker/index.php?func=detail&aid=1904505&group_id=167309&atid=842434">bug
     *      #1904505: magic sets: negative rules with negative query
     *      literals</a>
     */
	...
	// magic_p_ff() :- .
	rules.add(BASIC.createRule(Arrays.asList(createMagicLiteral("p", FF,
		new ITerm[] {})), Collections.<ILiteral> emptyList()));
	// magic_q_bb(?X, 3) :- p(?X, ?Y).
	rules.add(BASIC.createRule(Arrays
		.asList(createMagicLiteral("q", BB, X3)), Arrays
		.asList(createLiteral("p", "X", "Y"))));
	// p(?X, ?Y) :- a(?X, ?Y).
	...
	rules.add(BASIC.createRule(Arrays.asList(createLiteral("q", "X", "Y")),
		Arrays.asList(createMagicLiteral("q", BB, XY), createLiteral(
			"b", "X", "Y"))));

	// ?- p(?X, ?Y), !q(?X, 3).
	final IQuery query = BASIC.createQuery(Arrays.asList(createLiteral("p",
		"X", "Y"), BASIC.createLiteral(false, BASIC.createPredicate(
		"q", 2), BASIC.createTuple(X3))));
     ...
     * 
     * @see <a
     *      href="http://sourceforge.net/tracker/index.php?func=detail&aid=1907086&group_id=167309&atid=842434">bug
     *      #1907086: magic sets: labeled rules are not consturced correctly</a>
     */
	...
	rules.add(BASIC.createRule(Arrays.asList(createLiteral("a", "X", "Y")),
		Arrays.asList(createMagicLiteral("a", BF, new ITerm[] { X }),
			createLiteral("b", "X", "Y"), createLiteral(false, "c",
				"X", "Y"))));
	...
	rules.add(BASIC.createRule(Arrays.asList(createLiteral("c", "X", "Y")),
		Arrays.asList(createMagicLiteral("c", BB, XY), createLiteral(
			"e", "X", "Y"))));
	// magic_a_bf(2) :- .
	rules.add(seedRule(createMagicLiteral("a", BF, new ITerm[] { CONCRETE
		.createInteger(2) })));
	// magic_b_bf(?X) :- magic_a_bf(?X).
	rules.add(BASIC.createRule(Arrays.asList(createMagicLiteral("b", BF,
		new ITerm[] { X })), Arrays.asList(createMagicLiteral("a", BF,
		new ITerm[] { X }))));
	// label_c_1_bf(?X) :- magic_a_bf(?X).
	rules.add(BASIC.createRule(Arrays.asList(createLabeledLiteral("c_1",
		BF, new ITerm[] { X })), Arrays.asList(createMagicLiteral("a",
		BF, new ITerm[] { X }))));
	// label_c_2_bb(?X, ?Y) :- magic_a_bf(?X), b(?X, ?Y).
	rules.add(BASIC.createRule(Arrays.asList(createLabeledLiteral("c_2",
		BB, XY)), Arrays.asList(createMagicLiteral("a", BF,
		new ITerm[] { X }), createLiteral("b", "X", "Y"))));
	// magic_c_bb(?X, ?Y) :- label_c_1_bf(?X), label_c_2_bb(?X, ?Y).
	rules.add(BASIC.createRule(Arrays
		.asList(createMagicLiteral("c", BB, XY)), Arrays.asList(
		createLabeledLiteral("c_1", BF, new ITerm[] { X }),
		createLabeledLiteral("c_2", BB, XY))));

	// construct the query
	// ?- a(2, ?Y).
	final IQuery query = BASIC.createQuery(Arrays.asList(BASIC
		.createLiteral(true, BASIC.createPredicate("a", 2), BASIC
			.createTuple(CONCRETE.createInteger(2), Y))));
     ...
     * transformation.
     * 
     * @see <a href="https://sourceforge.net/tracker/index.php?func=detail&aid=1919554&group_id=167309&atid=842434"
	...
	rules.add(BASIC.createRule(Arrays.asList(createLiteral("a", "X", "Y")),
		Arrays.asList(createMagicLiteral("a", BF, new ITerm[] { X }),
			createLiteral("b", "X", "Z"), BASIC.createLiteral(true,
				BUILTIN.createAddBuiltin(Z, CONCRETE
					.createInteger(1), Y)))));
	// magic_a_bf('john') :- .
	...
	// ?- a('john'), EQUAL(?X, 'john').
	final IQuery query = BASIC.createQuery(Arrays.asList(BASIC
		.createLiteral(true, BASIC.createPredicate("a", 1), BASIC
			.createTuple(john)), BASIC.createLiteral(true, BUILTIN
		.createEqual(X, john))));
    ...
    /**
     * Prints a program and the resulting magic program in a formated way.
     * 
     * @param name
     *            the name to identify the test
     * @param prog
     *            the input program
     * @param result
     *            the magic set result
     */
    ...
    /**
     * Prints out a program, resulting magic program and the expected result.
     * 
     * @param name
     *            the name to identify the printing
     * @param prog
     *            the string representation of the input program
     * @param expected
     *            the expected transformation result
     * @param result
     *            the real outcome of the transformation
     */
    private static void printDebug(final String name, final String prog,
	...
	return new TestSuite(MagicSetsTest.class, MagicSetsTest.class
		.getSimpleName());
    }
}

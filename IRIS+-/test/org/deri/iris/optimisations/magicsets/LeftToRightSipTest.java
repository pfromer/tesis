...
public class LeftToRightSipTest extends TestCase {
	...
	return new TestSuite(LeftToRightSipTest.class, LeftToRightSipTest.class
		.getSimpleName());
    }
     ...
     */
    private static LeftToRightSip parseProgram(final String prog)
     ...
     * 
     * @param prog
     *            the program to parse containing the rule
     * @return the parsed rule
     */
    private static IRule parseSingleRule(final String prog)
	    throws ParserException {
	assert prog != null : "The program must not be null";
	...
	edges.add(createEdge(createLiteral("sg", "X", "Y"), createLiteral("up",
		"X", "Z1"), X));
	edges.add(createEdge(createLiteral("up", "X", "Z1"), createLiteral(
		"sg", "Z1", "Z2"), Z1));
	edges.add(createEdge(createLiteral("sg", "Z1", "Z2"), createLiteral(
		"flat", "Z2", "Z3"), Z2));
	edges.add(createEdge(createLiteral("flat", "Z2", "Z3"), createLiteral(
		"sg", "Z3", "Z4"), Z3));
	edges.add(createEdge(createLiteral("sg", "Z3", "Z4"), createLiteral(
		"down", "Z4", "Y"), Z4));
	...
	final Set<LabeledEdge<ILiteral, Set<IVariable>>> edges = new HashSet<LabeledEdge<ILiteral, Set<IVariable>>>();
	edges.add(createEdge(createLiteral("rsg", "X", "Y"), createLiteral(
		"up", "X", "X1"), X));
	edges.add(createEdge(createLiteral("up", "X", "X1"), createLiteral(
		"rsg", "Y1", "X1"), X1));
	edges.add(createEdge(createLiteral("rsg", "Y1", "X1"), createLiteral(
		"down", "Y1", "Y"), Y1));
	...
	edges.add(createEdge(createLiteral("up", "X", "X1"), createLiteral(
		"rsg", "Y1", "X1"), X1));
	edges.add(createEdge(createLiteral("rsg", "Y1", "X1"), BASIC
		.createLiteral(true, BUILTIN.createEqual(TERM
			.createVariable("Y1"), TERM.createVariable("Y"))), Y1));
     ...
     * </p>
     * <p>
     * <b>Note: Rules containing one literal multiple times will only contain
     * one edge representing it, no matter, how often such a literal occurs in
     * the rule.</b>
     * </p>
     */
    ...
    /**
     * Ensures, that negative literals don't produce variable passings, but can
     * receive one.
     */
    ...
    /**
     * Checks whether the dependency retrieval of a literal depending on itself
     * succeeds.
     */
	...
	assertEquals(ax + " depends on itself.", Collections.singleton(ax), sip
		.getDepends(ax));
    }
	...
	final Set<IVariable> bound_up = new HashSet<IVariable>(Arrays
		.asList(new IVariable[] { X }));
	final Set<IVariable> bound_sg = new HashSet<IVariable>(Arrays
		.asList(new IVariable[] { Z1 }));
	final Set<IVariable> bound_down = new HashSet<IVariable>(Arrays
		.asList(new IVariable[] { X, Z1, Z2 }));
	final Set<IVariable> bound_again = new HashSet<IVariable>(Arrays
		.asList(new IVariable[] { X, Z1, Z3 }));
	...
	assertEquals("Bounds of down wrong", bound_down, sip
		.getBoundVariables(down));
	assertEquals("Bounds of again wrong", bound_again, sip
		.getBoundVariables(again));
    }
	...
	final Set<IVariable> bound_up = new HashSet<IVariable>(Arrays
		.asList(new IVariable[] { X }));
	final Set<IVariable> bound_rsg0 = new HashSet<IVariable>(Arrays
		.asList(new IVariable[] { X1 }));
	final Set<IVariable> bound_down = new HashSet<IVariable>(Arrays
		.asList(new IVariable[] { Y1 }));
	...
	assertEquals("Bounds of up wrong", bound_up, sip.getBoundVariables(up));
	assertEquals("Bounds of rsg0 wrong", bound_rsg0, sip
		.getBoundVariables(rsg0));
	assertEquals("Bounds of down wrong", bound_down, sip
		.getBoundVariables(down));
    }
    ...
    /**
     * Tests the behaviour of the literal comparator with literals which gain
     * and give no passings.
     */
	...
	final String prog = "i0(?H0) :- e1(?H0), i0(?X0)." + "?- i0(11).";
	final LeftToRightSip sip = parseProgram(prog);
	...
	assertEquals(
		"getBoundVariables(...) must return an empty set for unconnected literals.",
		Collections.<IVariable> emptySet(), sip.getBoundVariables(i0x0));
	assertEquals(
		"getDepends(...) must return an empty set for unconnected literals.",
		Collections.<ILiteral> emptySet(), sip.getDepends(i0x0));
	assertEquals(
		"getEdgesEnteringLiteral(...) must return an empty set for unconnected literals.",
		Collections.<LabeledEdge<ILiteral, Set<IVariable>>> emptySet(),
		sip.getEdgesEnteringLiteral(i0x0));
	assertEquals(
		"getEdgesLeavingLiteral(...) must return an empty set for unconnected literals.",
		Collections.<LabeledEdge<ILiteral, Set<IVariable>>> emptySet(),
		sip.getEdgesLeavingLiteral(i0x0));
	assertFalse(
		"containsVertex(...) must return false for unconnected literals.",
		sip.containsVertex(i0x0));
	assertEquals(
		"variablesPassedByLiteral(...) must return an empty set for unconnected literals #1.",
		Collections.<ILiteral> emptySet(), sip
			.variablesPassedByLiteral(i0h0, i0x0));
	assertEquals(
		"variablesPassedByLiteral(...) must return an empty set for unconnected literals #2.",
		Collections.<ILiteral> emptySet(), sip
			.variablesPassedByLiteral(i0x0, i0h0));
    }

    /**
     * Tests the correct behaviour with rules with multiple head literals and
     * queries with multiple literals.
     */
	...
	final Set<LabeledEdge<ILiteral, Set<IVariable>>> referenceEdges = new HashSet<LabeledEdge<ILiteral, Set<IVariable>>>();
	referenceEdges.add(createEdge(a, d, A));
	referenceEdges.add(createEdge(c, e, E));
	referenceEdges.add(createEdge(d, e, B));

	assertEquals("The edge set does not match.", referenceEdges, sip
		.getEdges());
    }
	...
	final String prog = "i(?X) :- ?X < ?Y, e(?Y)." + "?- i(?X).";
	final LeftToRightSip sip = parseProgram(prog);
	assertEquals("There must be not edges created", Collections
		.<LabeledEdge<ILiteral, Set<IVariable>>> emptySet(), sip
		.getEdges());
    }
	...
	final ILiteral less = BASIC.createLiteral(true, BUILTIN
		.createLess(X, Y));

 ...
 * @version 0.1b
 */
public class CoverGraphTest extends TestCase {
  ...
  private final Logger LOGGER = Logger.getLogger(CoverGraphTest.class);
   ...
   * True Outcome
   */
  @Test public void testCoverGraphLinearAcyclicSingleWay() throws Exception {
    ...
    // final Set<List<IRule>> slr2r4 = ImmutableSet.of(lr2r4);
    // cmp.put(ImmutablePair.of(t2, m1), slr2r4);

    final Map<Pair<IPosition, IPosition>, Set<Pair<List<IPosition>, List<IRule>>>> propGraph = DepGraphUtils
        .computePropagationGraph(in);

    final Map<Pair<IPosition, IPosition>, Set<Pair<List<IPosition>, List<IRule>>>> covGraph = DepGraphUtils
        .computeCoverGraph(propGraph);

    LOGGER.info(IOUtils.LINE_SEPARATOR + "Propagation Graph:" + IOUtils.LINE_SEPARATOR
        + DepGraphUtils.prettyPrintPositionGraph(propGraph));
    ...
    LOGGER.info(IOUtils.LINE_SEPARATOR + "Actual:" + IOUtils.LINE_SEPARATOR
        + DepGraphUtils.prettyPrintPositionGraph(covGraph));

    assertEquals(true, covGraph.equals(cmp));
  ...
  @Test public void testCyclicRepeatedSuffix() throws Exception {
    ...
    final IPredicate t = Factory.BASIC.createPredicate("t", 1);
    final IPredicate s = Factory.BASIC.createPredicate("s", 2);
    final IPredicate r = Factory.BASIC.createPredicate("r", 2);

    final ITerm x = Factory.TERM.createVariable("X");
    final ITerm y = Factory.TERM.createVariable("Y");

    final ILiteral tx = Factory.BASIC.createLiteral(true, t, Factory.BASIC.createTuple(x));
    final ILiteral sxy = Factory.BASIC.createLiteral(true, s, Factory.BASIC.createTuple(x, y));
    final ILiteral rxy = Factory.BASIC.createLiteral(true, r, Factory.BASIC.createTuple(x, y));

    // Input structure

    final IRule r2 = Factory.BASIC.createRule(ImmutableList.of(sxy), ImmutableList.of(tx));
    final IRule r3 = Factory.BASIC.createRule(ImmutableList.of(tx), ImmutableList.of(sxy));
    final IRule r4 = Factory.BASIC.createRule(ImmutableList.of(tx), ImmutableList.of(rxy));
    final IRule r5 = Factory.BASIC.createRule(ImmutableList.of(rxy), ImmutableList.of(tx));

    final List<IRule> in = ImmutableList.of(r2, r3, r4, r5);
    ...
    // //
    //
    // final Map<Pair<IPosition, IPosition>, Set<List<IRule>>> cmp = new HashMap<Pair<IPosition, IPosition>,
    // Set<List<IRule>>>();
    //
    // final IPosition d1 = new Position(d.getPredicateSymbol(), 1);
    // final IPosition d2 = new Position(d.getPredicateSymbol(), 2);
  ...
  @Test public void testCyclicRepeatedSuffix2() throws Exception {
    ...
    final ILiteral tx = Factory.BASIC.createLiteral(true, t, Factory.BASIC.createTuple(x));
    final ILiteral sxx = Factory.BASIC.createLiteral(true, s, Factory.BASIC.createTuple(x, x));

    // Input structure

    final IRule r1 = Factory.BASIC.createRule(ImmutableList.of(sxx), ImmutableList.of(tx));
    final IRule r2 = Factory.BASIC.createRule(ImmutableList.of(tx), ImmutableList.of(sxx));

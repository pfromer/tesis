...
public class AddBuiltinTest extends TestCase {
	...
	assertEquals(BASIC.createTuple(T_1), b_x56.evaluate(BASIC.createTuple(
		X, X, X)));
	// 1 + X = 6
	final AddBuiltin b_1x6 = new AddBuiltin(T_1, X, T_6);
	assertEquals(BASIC.createTuple(T_5), b_1x6.evaluate(BASIC.createTuple(
		X, X, X)));
	// 1 + 5 = X
	final AddBuiltin b_15x = new AddBuiltin(T_1, T_5, X);
	assertEquals(BASIC.createTuple(T_6), b_15x.evaluate(BASIC.createTuple(
		X, X, X)));
	// 1 + X = Y
	final AddBuiltin b_1xy = new AddBuiltin(T_1, X, Y);
	assertEquals(BASIC.createTuple(T_5), b_1xy.evaluate(BASIC.createTuple(
		X, X, T_6)));
	assertEquals(BASIC.createTuple(T_6), b_1xy.evaluate(BASIC.createTuple(
		X, T_5, X)));
	// X + 5 = Y
	final AddBuiltin b_x5y = new AddBuiltin(X, T_5, Y);
	assertEquals(BASIC.createTuple(T_1), b_x5y.evaluate(BASIC.createTuple(
		X, X, T_6)));
	assertEquals(BASIC.createTuple(T_6), b_x5y.evaluate(BASIC.createTuple(
		T_1, X, X)));
	// X + Y = 6
	final AddBuiltin b_xy6 = new AddBuiltin(X, Y, T_6);
	assertEquals(BASIC.createTuple(T_1), b_xy6.evaluate(BASIC.createTuple(
		X, T_5, X)));
	assertEquals(BASIC.createTuple(T_5), b_xy6.evaluate(BASIC.createTuple(
		T_1, X, X)));
	// X + Y = Z
	final AddBuiltin b_xyz = new AddBuiltin(X, Y, Z);
	assertEquals(BASIC.createTuple(T_1), b_xyz.evaluate(BASIC.createTuple(
		X, T_5, T_6)));

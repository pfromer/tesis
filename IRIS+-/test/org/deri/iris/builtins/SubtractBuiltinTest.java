...
public class SubtractBuiltinTest extends TestCase {
	...
	return new TestSuite(SubtractBuiltinTest.class,
		SubtractBuiltinTest.class.getSimpleName());
    }
	...
	assertEquals(BASIC.createTuple(T_11), b_x56.evaluate(BASIC.createTuple(
		X, X, X)));
	// 11 - X = 6
	final SubtractBuiltin b_11x6 = new SubtractBuiltin(T_11, X, T_6);
	assertEquals(BASIC.createTuple(T_5), b_11x6.evaluate(BASIC.createTuple(
		X, X, X)));
	// 11 - 5 = X
	final SubtractBuiltin b_115x = new SubtractBuiltin(T_11, T_5, X);
	assertEquals(BASIC.createTuple(T_6), b_115x.evaluate(BASIC.createTuple(
		X, X, X)));
	// 11 - X = Y
	final SubtractBuiltin b_11xy = new SubtractBuiltin(T_11, X, Y);
	assertEquals(BASIC.createTuple(T_5), b_11xy.evaluate(BASIC.createTuple(
		X, X, T_6)));
	assertEquals(BASIC.createTuple(T_6), b_11xy.evaluate(BASIC.createTuple(
		X, T_5, X)));
	// X - 5 = Y
	final SubtractBuiltin b_x5y = new SubtractBuiltin(X, T_5, Y);
	assertEquals(BASIC.createTuple(T_11), b_x5y.evaluate(BASIC.createTuple(
		X, X, T_6)));
	assertEquals(BASIC.createTuple(T_6), b_x5y.evaluate(BASIC.createTuple(
		T_11, X, X)));
	// X - Y = 6
	final SubtractBuiltin b_xy6 = new SubtractBuiltin(X, Y, T_6);
	assertEquals(BASIC.createTuple(T_11), b_xy6.evaluate(BASIC.createTuple(
		X, T_5, X)));
	assertEquals(BASIC.createTuple(T_5), b_xy6.evaluate(BASIC.createTuple(
		T_11, X, X)));
	// X - Y = Z
	final SubtractBuiltin b_xyz = new SubtractBuiltin(X, Y, Z);
	assertEquals(BASIC.createTuple(T_11), b_xyz.evaluate(BASIC.createTuple(
		X, T_5, T_6)));
	assertEquals(BASIC.createTuple(T_5), b_xyz.evaluate(BASIC.createTuple(
		T_11, X, T_6)));
	assertEquals(BASIC.createTuple(T_6), b_xyz.evaluate(BASIC.createTuple(
		T_11, T_5, X)));

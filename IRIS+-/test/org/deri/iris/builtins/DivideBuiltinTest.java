...
public class DivideBuiltinTest extends TestCase {
	...
	assertEquals(BASIC.createTuple(T_10), b_x52.evaluate(BASIC.createTuple(
		X, X, X)));
	// 10 / X = 2
	final DivideBuiltin b_10x2 = new DivideBuiltin(T_10, X, T_2);
	assertEquals(BASIC.createTuple(T_5), b_10x2.evaluate(BASIC.createTuple(
		X, X, X)));
	// 10 / 5 = X
	final DivideBuiltin b_105x = new DivideBuiltin(T_10, T_5, X);
	assertEquals(BASIC.createTuple(T_2), b_105x.evaluate(BASIC.createTuple(
		X, X, X)));
	// 10 / X = Y
	final DivideBuiltin b_10xy = new DivideBuiltin(T_10, X, Y);
	assertEquals(BASIC.createTuple(T_5), b_10xy.evaluate(BASIC.createTuple(
		X, X, T_2)));
	assertEquals(BASIC.createTuple(T_2), b_10xy.evaluate(BASIC.createTuple(
		X, T_5, X)));
	// X / 5 = Y
	final DivideBuiltin b_x5y = new DivideBuiltin(X, T_5, Y);
	assertEquals(BASIC.createTuple(T_10), b_x5y.evaluate(BASIC.createTuple(
		X, X, T_2)));
	assertEquals(BASIC.createTuple(T_2), b_x5y.evaluate(BASIC.createTuple(
		T_10, X, X)));
	// X / Y = 2
	final DivideBuiltin b_xy2 = new DivideBuiltin(X, Y, T_2);
	assertEquals(BASIC.createTuple(T_10), b_xy2.evaluate(BASIC.createTuple(
		X, T_5, X)));
	assertEquals(BASIC.createTuple(T_5), b_xy2.evaluate(BASIC.createTuple(
		T_10, X, X)));
	// X / Y = Z
	final DivideBuiltin b_xyz = new DivideBuiltin(X, Y, Z);
	assertEquals(BASIC.createTuple(T_10), b_xyz.evaluate(BASIC.createTuple(
		X, T_5, T_2)));
	assertEquals(BASIC.createTuple(T_5), b_xyz.evaluate(BASIC.createTuple(
		T_10, X, T_2)));
	assertEquals(BASIC.createTuple(T_2), b_xyz.evaluate(BASIC.createTuple(
		T_10, T_5, X)));

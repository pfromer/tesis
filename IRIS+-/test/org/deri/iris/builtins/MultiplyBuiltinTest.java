...
public class MultiplyBuiltinTest extends TestCase {
	...
	assertEquals(BASIC.createTuple(T_2), b_x510.evaluate(BASIC.createTuple(
		X, X, X)));
	// 2 * X = 10
	final MultiplyBuiltin b_2x10 = new MultiplyBuiltin(T_2, X, T_10);
	assertEquals(BASIC.createTuple(T_5), b_2x10.evaluate(BASIC.createTuple(
		X, X, X)));
	// 2 * 5 = X
	final MultiplyBuiltin b_25x = new MultiplyBuiltin(T_2, T_5, X);
	assertEquals(BASIC.createTuple(T_10), b_25x.evaluate(BASIC.createTuple(
		X, X, X)));
	// 2 * X = Y
	final MultiplyBuiltin b_2xy = new MultiplyBuiltin(T_2, X, Y);
	assertEquals(BASIC.createTuple(T_5), b_2xy.evaluate(BASIC.createTuple(
		X, X, T_10)));
	assertEquals(BASIC.createTuple(T_10), b_2xy.evaluate(BASIC.createTuple(
		X, T_5, X)));
	// X * 5 = Y
	...
	assertEquals(BASIC.createTuple(T_2), b_xy10.evaluate(BASIC.createTuple(
		X, T_5, X)));
	assertEquals(BASIC.createTuple(T_5), b_xy10.evaluate(BASIC.createTuple(
		T_2, X, X)));
	// X * Y = Z

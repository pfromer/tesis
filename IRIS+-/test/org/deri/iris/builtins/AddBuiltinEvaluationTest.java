...
public class AddBuiltinEvaluationTest extends TestCase {
	...
	// p(X) :- r(X)
	IRule r = Factory.BASIC.createRule(Arrays
		.asList(createLiteral("p", "X")), Arrays.asList(createLiteral(
		"r", "X")));
	rules.add(r);
	// p(X) :- s(X), add(3, 4, X)
	List<ILiteral> h = Arrays.asList(createLiteral("p", "X"));
	List<ILiteral> b = Arrays.asList(createLiteral("s", "X"), Factory.BASIC
		.createLiteral(true, Factory.BUILTIN.createAddBuiltin(CONCRETE
			.createInteger(3), CONCRETE.createInteger(4), TERM
			.createVariable("X"))));
	...
	IRelation rel = new RelationFactory().createRelation();
	...
	rel = new RelationFactory().createRelation();
	...
	// Result: p(1), p(2), p(7)
	IRelation res = new RelationFactory().createRelation();
	...
	List<ILiteral> b = Arrays.asList(createLiteral("s", "X"),
		createLiteral("p", "Y"), Factory.BASIC.createLiteral(true,
			Factory.BUILTIN.createAddBuiltin(TERM
	...
	IRelation res = new RelationFactory().createRelation();
	res.add(BASIC.createTuple(CONCRETE.createInteger(3), CONCRETE
		.createInteger(1), CONCRETE.createInteger(4)));
	res.add(BASIC.createTuple(CONCRETE.createInteger(3), CONCRETE
		.createInteger(2), CONCRETE.createInteger(5)));
	res.add(BASIC.createTuple(CONCRETE.createInteger(7), CONCRETE
		.createInteger(1), CONCRETE.createInteger(8)));
	res.add(BASIC.createTuple(CONCRETE.createInteger(7), CONCRETE
		.createInteger(2), CONCRETE.createInteger(9)));
	...
	List<ILiteral> h = Arrays.asList(createLiteral("q", "X", "Y", "Z"));
	List<ILiteral> b = Arrays.asList(Factory.BASIC.createLiteral(true,
		Factory.BUILTIN.createAddBuiltin(TERM.createVariable("X"),
			CONCRETE.createInteger(4), TERM.createVariable("Z"))),
		createLiteral("s", "X"), createLiteral("p", "Y"), Factory.BASIC
			.createLiteral(true, Factory.BUILTIN.createAddBuiltin(
				TERM.createVariable("X"), TERM
					.createVariable("Y"), CONCRETE
					.createInteger(10))));
	...
	// Result: q(6,4,10)
	IRelation res = new RelationFactory().createRelation();
	res.add(BASIC.createTuple(CONCRETE.createInteger(6), CONCRETE
		.createInteger(4), CONCRETE.createInteger(10)));
     ...
     *             if the predicate name or the set of variable names is {@code
     *             null}
     * @throws NullPointerException

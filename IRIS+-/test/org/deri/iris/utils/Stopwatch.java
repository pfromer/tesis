...
public class Stopwatch {
     ...
     * 
     * @param startIndex
     *            the index from where to start (inclusive)
     * @param stopIndex
     *            the index where to stop (exclusive)
     * @return the added up time spans
     * @throws IllegalArgumentException
     *             if the start in index is greater than the stop index
     */
    public long timeDifference(final int startIndex, final int stopIndex) {
	if (startIndex > stopIndex) {
	    throw new IllegalArgumentException(
		    "The start index must not be greater than the stop index");
	}
     ...
     * 
     * @param hundredPercentIndex
     *            the mark denoting 100%
     * @param relativePercentIndex
     *            the mark for which to calculate the percentage in relation to
     *            the 100% mark
     * @return the percentage of time used of the relative mark in relation to
     *         the 100% mark
     */
    public float timePercentage(final int hundredPercentIndex,
	    final int relativePercentIndex) {
	final long hundredPercentMillis = getMark(hundredPercentIndex)
		.getMilliseconds();
	final long relativePercentMillis = getMark(relativePercentIndex)
		.getMilliseconds();
     ...
     */
    public String formattedTimePercentage(final int hundredPercentIndex,
	    final int relativePercentIndex) {
	...
	return new Formatter().format(
		"%s (%dms) took %.2f%% to complete in relation to %s (%dms)",
	 ...
	 * @throws IllegalArgumentException
	 *             if the milliseconds are negative
	 * @throws IllegalArgumentException
	 *             if the name is <code>null</code>
	 */
	public Mark(final long milliseconds, final String name) {
	    if (milliseconds < 0) {
		throw new IllegalArgumentException(
			"The milliseconds must not be negative");
	    }
	    if (name == null) {
	...
	public boolean equals(final Object other) {
	    ...
	    return milliseconds == mark.milliseconds && name.equals(mark.name);
	}

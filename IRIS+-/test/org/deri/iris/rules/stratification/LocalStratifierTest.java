...
public class LocalStratifierTest extends TestCase {
    ILiteral createLiteral(boolean positive, String predicateName,
	    Object... termObjects) {
	...
	if (predicateName.equals("="))
	    return BASIC.createLiteral(positive, new EqualBuiltin(terms
		    .toArray(new ITerm[2])));

	if (predicateName.equals("!="))
	    return BASIC.createLiteral(positive, new NotEqualBuiltin(terms
		    .toArray(new ITerm[2])));

	if (predicateName.equals("<"))
	    return BASIC.createLiteral(positive, new LessBuiltin(terms
		    .toArray(new ITerm[2])));

	return BASIC.createLiteral(positive, BASIC.createAtom(BASIC
		.createPredicate(predicateName, terms.size()), BASIC
		.createTuple(terms)));
    }
	...
	IRule p = createRule(createLiteral(true, "p", 1, "Y"), createLiteral(
		true, "q", "X", "Y"), createLiteral(false, "p", 2, "Y"));
	...
	IRule p = createRule(createLiteral(true, "p", 1, "Y"), createLiteral(
		true, "r", "Y"), createLiteral(false, "q", 2, "Y"));

	IRule q = createRule(createLiteral(true, "q", "X", "Y"), createLiteral(
		true, "p", "X", "Y"));
	rules.add(p);
	rules.add(q);
	...
	IRule p = createRule(createLiteral(true, "p", "X", "Y"), createLiteral(
		true, "r", "X", "Y"), createLiteral(false, "p", 2, "Y"),
	...
	IRule p = createRule(createLiteral(true, "p", 2, "Y"), createLiteral(
		true, "r", "Y"), createLiteral(false, "q", 2, "Y"));

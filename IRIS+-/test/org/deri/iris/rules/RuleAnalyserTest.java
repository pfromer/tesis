...
public class RuleAnalyserTest extends TestCase {
	...
	return new TestSuite(RuleAnalyserTest.class, RuleAnalyserTest.class
		.getSimpleName());
    }
	...
	for (final String rule : illegalArguemntExceptionRules) {
		...
		RuleAnalyser.hasHeadLiteralInBody(parseRule(rule));
		fail(rule
			+ " is supposed to throw an IllegalArgumentException.");
    ...
    public void testUnsatisfiableVariableAssignment() throws ParserException,
	    EvaluationException {
	...
	for (final String rule : trueRules) {
	    assertFalse("hasSatisfiableVariableAssignment returns true for \""
		    + rule + "\".", RuleAnalyser
		    .hasSatisfiableVariableAssignment(parseRule(rule)));
	}
    }

    public void testSatisfiableVariableAssignment() throws ParserException,
	    EvaluationException {
	final String[] falseRules = new String[] {
		...
		"p(?X) :- q(?X), r(?X), ! f(3, ?X) != f(3, ?X).", };
	for (final String rule : falseRules) {
	    assertTrue("hasSatisfiableVariableAssignment returns false for \""
		    + rule + "\".", RuleAnalyser
		    .hasSatisfiableVariableAssignment(parseRule(rule)));
	}
    }

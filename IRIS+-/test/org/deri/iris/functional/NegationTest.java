...
public class NegationTest extends TestCase {
	...
	String program = "s('d')." + "s('b')." + "s('a')." + "s('q')." +
	...
	String program = "u('d')." + "u('b')." + "u('a')." + "u('q')." +
	...
	"p(?X) :- q(?X), not r(?X)." + "r(?X) :- s(?X), not t(?X)."
		+ "t(?X) :- u(?X)." + "?- p(?X).";

	String expectedResults = "p('a')." + "p('b')." + "p('e').";

	Helper.evaluateWithAllStrategies(program, expectedResults);
    }

    /**
     * This test is to ensure that both rules are propely stratified. The rule
     * with the empty body could be in stratum 0, but the second rule means it
     * must be one higher than predicate 's'.
     * 
     * @throws Exception
     */
    public void testGloballyStratifiedEmptyRuleBody() throws Exception {
	String program = "p('a'):-." + "p(?X) :- r(?X), not s(?Y)."
		+ "?- p(?X).";
     ...
     * i.e. positive literals first, even when the negative literal appears
     * first.
     * 
     * @throws Exception
     */
    public void testNegatedSubGoalFirst() throws Exception {
	String program = "r('a', 'a')." + "r('b', 'a')." +

	"s('a', 'a')." + "s('b', 'b')." +

	"p(?X) :- not s(?X, 'a'), r(?X, ?Y)." + "?- p(?X).";

	String expectedResults = "p('b').";
	...
	String program = "p( ?X ) :- r( ?X ), not q(?X).";
	...
	String program = "p1( ?X ) :- r( ?X ), not q(?X)."
		+ "p2( ?X ) :- r( ?X ), not q(?X).";
	...
	String program = "p( ?X ) :- r( ?X ), s(?X)."
		+ "p( ?X ) :- r( ?X ), t(?X)." + "p( ?X ) :- s( ?X ), t(?X)."
		+ "p( ?X ) :- not u( ?X ), t(?X)."
		+ "p( ?X ) :- s(?X), u( ?X )." +

		"u( ?X ) :- s(?X)." + "u( ?X ) :- t(?X)." + "u( ?X ) :- v(?X).";
	...
	Helper.checkFailureWithNaive(program,
		ProgramNotStratifiedException.class);
	Helper.checkFailureWithSemiNaive(program,
		ProgramNotStratifiedException.class);
    }
	...
	String program = "p1( ?X ) :- r( ?X ),not q(?X)."
		...
		"p7( ?X ) :- t(?X), not p6(?X)." + "p6( ?X ) :- t(?X), p7(?X).";
	...
	String program = "p( ?X ) :- r( ?X ), s(?X)."
		+ "p( ?X ) :- r( ?X ), t(?X)." + "p( ?X ) :- s( ?X ), t(?X)."
		+ "p( ?X ) :- not u( ?X ), t(?X)."
		+ "p( ?X ) :- s(?X), u( ?X )." + "p( ?X ) :- s(?X), w( ?X )." +

		"u( ?X ) :- s(?X)." + "u( ?X ) :- w(?X)."
		+ "u( ?X ) :- t(?X), not p(?X)." + "u( ?X ) :- x(?X)."
		+ "u( ?X ) :- v(?X).";
	...
	String program = "p1( ?X ) :- r( ?X ),not p8(?X)."
		+ "p2( ?X ) :- r( ?X ),not p1(?X)."
		+ "p3( ?X ) :- r( ?X ),not p2(?X)."
		+ "p4( ?X ) :- r( ?X ),not p3(?X)."
		+ "p5( ?X ) :- r( ?X ),not p4(?X)."
	...
	String program = "r(0)." + "r(1)." + "r(2)." + "r(3)." + "r(4)." +

	"v(?X) :- r(?X), not  ?X > 2." + "w(?X) :- r(?X),      ?X > 2.";

	Helper.evaluateWithAllStrategies(program + "?- v(?X).",
		"v(0).v(1).v(2).");
	...
	String program = "r(1)." + "r(2)." + "r(3)." + "r(4)." +
	...
	String expectedResults = "v(1,6)." + "v(2,5).";

	Helper.evaluateWithAllStrategies(program + "?- v(?X,?Y).",
		expectedResults);

	expectedResults = "w(1,5)." + "w(1,7)." + "w(1,8)." + "w(2,6)."
		+ "w(2,7)." + "w(2,8)." + "w(3,5)." + "w(3,6)." + "w(3,7)."
		+ "w(3,8)." + "w(4,5)." + "w(4,6)." + "w(4,7)." + "w(4,8).";

	Helper.evaluateWithAllStrategies(program + "?- w(?X,?Y).",
		expectedResults);
    }
	...
	"p('a', ?X) :- s(?X), not p('b', ?X)." + "?- p(?X,?Y).";

	String expectedResults = "p('b', 2)." + "p('b', 4)." + "p('a', 1)."
		+ "p('a', 3).";

	Helper.evaluateWithAllStrategies(program, expectedResults);
    }

    public void testLocallyStratified_SelfDependency_NotEqualFromRuleBody()
	    throws Exception {
	String program = "s(1)." + "s(2)." + "s(3)." + "s(4)." +
	...
	String expectedResults = "p('b', 2)." + "p('b', 4).";

	Helper.evaluateWithAllStrategies(program, expectedResults);
    }

    public void testLocallyStratified_SelfDependency_NotEqualFromRuleBody2()
	    throws Exception {
	String program = "s(1)." + "s(2)." + "s(3)." + "s(4)." +
	...
	String expectedResults = "p('a', 1)." + "p('a', 3).";

	Helper.evaluateWithAllStrategies(program, expectedResults);
    }

    public void testLocallyStratified_DoubleNegativeDependency()
	    throws Exception {
	String program = "r(1)." + "r(2)." + "r(3)." + "r(4)." +
	...
	"p('b',?X) :- r(?X), not q('b',?X)." + "q(?X,?Y) :- p(?X,?Y).";
	...
	String program = "p('b', ?X) :- q(?X,?Y), ! p(?X,?Y).";
	...
	String program = "p('b',?X) :- r(?X), not q('c',?X)."
		+ "p(?X, ?Y) :- r(?X), p(?X, ?Y)." +
	...
	String program = "p(1,?x) :- r(?x), not q(2,?x)." +
	...
	String program = "r(1, 2)." + "r(2, 3)." + "r(3, 4)." + "r(4, 5)." +
	...
	String expectedResults = "p(1, 2)." + "p(3, 4)." + "p(4, 5).";
    ...
    public void testRuleWithOnlyNegatedGroundedLiteral_FactExists()
	    throws Exception {
	String program = "p('b') :- not r('a')." + "r('a')." + "?- p(?x).";
    ...
    public void testRuleWithOnlyNegatedGroundedLiteral_FactDoesNotExists()
	    throws Exception {
	String program = "p('b') :- not r('a')." + "r('b')." + "?- p(?x).";
    ...
    public void testRuleWithOnlyNegatedNonGroundLiteral_FactExists()
	    throws Exception {
	String program = "p('b') :- not r(?X)." + "r('a')." + "?- p(?x).";
    ...
    public void testRuleWithOnlyNegatedNonGroundLiteral_FactsDoesNotExist()
	    throws Exception {
	String program = "p('b') :- not r(?X)." + "?- p(?x).";
    ...
    public void testUnsafeRuleWithOnlyNegatedNonGroundLiteral()
	    throws Exception {
	String program = "p(?Y) :- not r(?X)." + "?- p(?x).";
	...
	String program = "person(1)." + "person(2)." + "person(3)."
		+ "person(4)." + "person(5)." + "person(6)." +

		"married(1,2)." + "married(3,4)." +
	...
	Helper.evaluateWithAllStrategies(program + "?- single(?X).",
		expectedResults);
	Helper.evaluateWithAllStrategies(program
		+ "?- person(?X), not married(?X, ?Y).", expectedResults);
    }
}

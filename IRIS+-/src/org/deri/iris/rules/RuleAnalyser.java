...
public class RuleAnalyser {
	 ...
	 * @return <code>true</code> if the rule body contains head atoms, otherwise <code>false</code>
	...
	public static boolean hasHeadLiteralInBody(final IRule rule) {
		...
		return modRule.getBody().contains(modRule.getHead().iterator().next());
	}
	 ...
	 * </p>
	 * <p>
	 * <b>The implementation of this method is not finished. At the moment it is not able to handle constructed
	 * terms.</b>
	 * </p>
	 * @param rule the rule to check
	 * @return <code>true</code> if a variable assignment was found, which doesn't make sense.
	 * @throws EvaluationException if something went wrong while evaluating one of the equality built-ins.
	 */
	...
	private static boolean unifies(final ITuple tuple) {
		assert tuple != null;
		assert tuple.size() == 2;
	...
	public static boolean containsConstructedTerms(final ITuple tuple) {
	 ...
	 * @param rule the rule to check
	 * @return <code>true</code> if the rule is productive, otherwise <code>false</code>
	 * @throws IllegalArgumentException if the rule is <code>null</code>
	 */

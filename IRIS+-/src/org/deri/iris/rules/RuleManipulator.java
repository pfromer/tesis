...
public class RuleManipulator {
	 ...
	 * @return The new rule.
	 */
	public IRule addEquality(final IRule rule, final ITerm term1, final ITerm term2) {
		// ILiteral literal = Factory.BASIC.createLiteral( true, new
		// EqualBuiltin( term1, term2 ) );
		final ILiteral literal = Factory.BASIC.createLiteral(true, new ExactEqualBuiltin(term1, term2));
	 ...
	 * @return The new rule.
	 */
	public IRule addInequality(final IRule rule, final ITerm term1, final ITerm term2) {
		// ILiteral literal = Factory.BASIC.createLiteral( true, new
		// UnEqualBuiltin( term1, term2 ) );
		final ILiteral literal = Factory.BASIC.createLiteral(true, new NotExactEqualBuiltin(term1, term2));
	 ...
	 * @return The new rule.
	 */
	public IRule addBodyLiteral(final IRule rule, final ILiteral literal) {
	 ...
	 */
	public IRule removeDuplicateLiterals(final IRule rule) {
		return Factory.BASIC.createRule(rule.getHead(), removeDuplicates(Lists.newLinkedList(rule.getBody())));
	}

	/**
	 * Iterate the rule looking for positive variable=variable equality built-ins. For each one found, traverse the
	 * whole rule and replace one variable with the other.
	 * @param rule The rule to process.
	 * @return The new rule.
	 */
			...
			for (final ILiteral literal : rule.getBody()) {
				if (literal.isPositive()) {
					if (literal.getAtom() instanceof EqualBuiltin) {
						final Tuple tuple = (Tuple) literal.getAtom().getTuple();
						...
						// If a positive equality between a variable and
						// variable then...
						if ((v1 != null) && (v2 != null)) {
							if (!v1.equals(v2)) {
								// ... re-write the rule replacing the variable
								// with the other variable
								final IRule rule2 = replace(rule, true, v1, v2);
	...
	/**
	 * Iterate the rule looking for positive variable==constant exact equalities. For each one found, replace the
	 * variable with the constant in the other rule literals.
	 * @param rule The rule to process
	 * @param strict True, do replacements only for exact equalities. False, use any equality.
	 * @return A new rule
	 */
	public IRule replaceVariablesWithConstants(IRule rule, final boolean strict) {
		boolean changed;
		...
		do {
			...
			for (final ILiteral literal : rule.getBody()) {
				...
				final boolean equality = literal.getAtom() instanceof EqualBuiltin;
				final boolean in_equality = literal.getAtom() instanceof NotEqualBuiltin;
				final boolean is = literal.getAtom() instanceof ExactEqualBuiltin;
				final boolean is_not = literal.getAtom() instanceof NotExactEqualBuiltin;
				...
				if (canReplace) {
					...
					if ((variable != null) && (constant != null)) {
						// ... re-write the rule replacing the variable with the
						// constant
						final IRule rule2 = replace(rule, true, variable, constant);
	 ...
	 * @return The new rule.
	 */
	public IRule removeUnnecessaryEqualityBuiltins(final IRule rule) {
		...
		for (final ILiteral literal : rule.getBody()) {
			...
			final boolean positive = literal.isPositive();
			final boolean equality = (literal.getAtom() instanceof EqualBuiltin)
			        || (literal.getAtom() instanceof ExactEqualBuiltin);
			final boolean in_equality = (literal.getAtom() instanceof NotEqualBuiltin)
			        || (literal.getAtom() instanceof NotExactEqualBuiltin);

			final boolean canReplace = (positive && equality) || (!positive && in_equality);
			...
			if (canReplace) {
				...
				final ITerm t1 = tuple.get(0);
				final ITerm t2 = tuple.get(1);
	 ...
	 * @return The new rule
	 */
	public IRule replace(final IRule rule, final boolean processHead, final ITerm remove, final ITerm replaceWith) {
	...
	private ILiteral replace(final ILiteral literal, final ITerm remove, final ITerm replacewith) {
		final boolean positive = literal.isPositive();
		final IAtom atom = replace(literal.getAtom(), remove, replacewith);

		return Factory.BASIC.createLiteral(positive, atom);
	}

	// public visibility needed for SLDNF resolution
	public IAtom replace(final IAtom atom, final ITerm remove, final ITerm replaceWith) {
		...
		final ITuple tuple = replace(atom.getTuple(), remove, replaceWith);

		return Factory.BASIC.createAtom(predicate, tuple);
	}

	private ITuple replace(final ITuple tuple, final ITerm remove, final ITerm replaceWith) {
		return Factory.BASIC.createTuple(replaceTerms(tuple, remove, replaceWith));
	}

	private ITerm[] replaceTerms(final ITuple tuple, final ITerm remove, final ITerm replaceWith) {
		final ITerm[] newTerms = new ITerm[tuple.size()];

		for (int t = 0; t < tuple.size(); ++t) {
			final ITerm oldTerm = tuple.get(t);
	...
	private IConstructedTerm replace(final IConstructedTerm constructed, final ITerm remove, final ITerm replaceWith) {
		final String functionSymbol = constructed.getFunctionSymbol();

		final List<ITerm> newTerms = new ArrayList<ITerm>();

		for (final ITerm oldTerm : constructed.getParameters()) {
			if (oldTerm.equals(remove)) {
	 ...
	 * @return The new atom.
	 */
	private IAtom replace(final IBuiltinAtom atom, final ITerm remove, final ITerm replaceWith) {
		final ITerm[] newTerms = replaceTerms(atom.getTuple(), remove, replaceWith);

		try {
			final Constructor<IBuiltinAtom> constructor = (Constructor<IBuiltinAtom>) atom.getClass().getConstructor(
			        newTerms.getClass());
			return constructor.newInstance((Object) newTerms);
		} catch (final Exception e) {
			// should never actually get here unless soemone's been fiddling
			// about.

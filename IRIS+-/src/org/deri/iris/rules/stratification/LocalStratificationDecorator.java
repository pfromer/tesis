...
public class LocalStratificationDecorator {
	...
	public enum MatchType {
		NONE, EXACT, CONSUMES_SUBSET, CONSUMES_ALL
	}
	...
	public static class Adornment {
		/**
		 * Find how well a term (from a dependant rule's negated sub-goal) matches the term decorated with this
		 * adornment.
		 * @param termFromNegatedSubgoal The 'interesting' negated sub goal from some rule
		 * @return An indication of how well this term can fulfil the corresponding term in the negated sub-goal
		 */
		public MatchType match(final ITerm termFromNegatedSubgoal) {
			if (termFromNegatedSubgoal.isGround()) {
				...
				// If 'this' is a list of what the rule can not produce for the
				// term
				for (final ITerm negatedTerm : mNegatedConstants) {
				...
				// termFromNegatedSubgoal is a variable, so we can't say
				// anything about it
		 ...
		 */
		public Adornment addNegatedConstant(final ITerm constantTerm) {
			assert !(constantTerm instanceof IVariable);

			final Adornment result = new Adornment();
		 ...
		 * @return The new adornment
		 */
		public Adornment setConstantTerm(final ITerm constantTerm) {
			assert !(constantTerm instanceof IVariable);
		...
		public String toString() {
			...
			for (final ITerm constant : mNegatedConstants) {
	 ...
	 */
	public LocalStratificationDecorator(final IRule rule, final List<Adornment> adornments) {
		assert rule.getHead().iterator().next().getAtom().getTuple().size() == adornments.size();
	 ...
	 */
	public MatchType match(final ITuple tuple) {
		assert mAdornments.size() == tuple.size();

		boolean partialmatch = false;

		for (int t = 0; t < mAdornments.size(); ++t) {
			final Adornment adornment = mAdornments.get(t);
			final ITerm term = tuple.get(t);

			final MatchType matchType = adornment.match(term);

...
/**
 * A local stratification algorithm. This algorithm will 'split' rules and so will likely return more rules than were
 * provided.
...
public class LocalStratifier implements IRuleStratifier {
	/**
	 * Constructor.
	 * @param strict true, if the stratifier is permitted to substitute variables with constants when they appear in
	 *            equality built-ins. false, if the substitution may only occur when the built-in is an exact equality
	 */
	public LocalStratifier(final boolean strict) {
		mStrict = strict;
	}
	...
	public List<List<IRule>> stratify(final List<IRule> rules) {
		...
		final int ruleStratum[] = new int[mRules.size()];

		final int ruleCount = mRules.size();
		int highest = 0;
		boolean change = true;
		final RuleManipulator rm = new RuleManipulator();
				...
				if (currentRule.getBody().size() == 0) {
					...
					for (final ILiteral bl : currentRule.getBody()) {
						for (int r2 = 0; r2 < mRules.size(); ++r2) {
							...
							final LocalStratificationDecorator adaptor = mRules.get(r2);

							if (adaptor.getRule().getHead().iterator().next().getAtom().getPredicate()
							        .equals(bl.getAtom().getPredicate())) {
								final MatchType match = adaptor.match(bl.getAtom().getTuple());
	...
	/**
	 * For every literal of every rule, if the literal is negative and not a built-in: split any dependent rules
	 */
	private void splitRules() {
		...
		do {
			...
			for (final LocalStratificationDecorator ruleAdaptor : mRules) {
				final IRule rule = ruleAdaptor.getRule();

				if (splitRulesForLiteralsFromOneRule(rule)) {
					// At least one rule has been split, so start again at the
					// beginning
					changed = true;
					break;
				}
	 ...
	 */
	private boolean splitRulesForLiteralsFromOneRule(final IRule currentRule) {
		// For each literal in the body
		for (final ILiteral literal : currentRule.getBody()) {
			...
			if (!literal.isPositive()) {
				...
				if (!atom.isBuiltin()) {
					...
					final List<IVariable> variables = tuple.getAllVariables();
					final boolean hasConstants = variables.size() != tuple.size();
	 ...
	 */
	private boolean splitRulesForAtom(final IAtom atom) {
		...
		do {
			...
			for (int ruleIndex = 0; ruleIndex < mRules.size(); ++ruleIndex) {
				final LocalStratificationDecorator decorator = mRules.get(ruleIndex);
				final IRule rule = decorator.getRule();

				// Predicate has same name and arity?
				if (atom.getPredicate().equals(rule.getHead().iterator().next().getAtom().getPredicate())) {
					final ITuple negatedSubGoalTuple = atom.getTuple();
					...
					if (decorator.match(negatedSubGoalTuple) == MatchType.CONSUMES_SUBSET) {
						...
						final LocalStratificationDecorator exactMatchRule = makeExactMatchRule(decorator,
						        negatedSubGoalTuple);
						final LocalStratificationDecorator noMatchRule = makeNoMatchRule(decorator, negatedSubGoalTuple);
	 ...
	 * @return The new rule.
	 */
	private LocalStratificationDecorator makeExactMatchRule(final LocalStratificationDecorator decorator,
	        final ITuple negatedSubGoalTuple) {
		IRule rule = decorator.getRule();
		final List<LocalStratificationDecorator.Adornment> adornments = decorator.getAdornments();
		final ITuple ruleHead = rule.getHead().iterator().next().getAtom().getTuple();
		...
		for (int t = 0; t < adornments.size(); ++t) {
			final ITerm headTerm = ruleHead.get(t);
			final ITerm subGoalTerm = negatedSubGoalTuple.get(t);
			final LocalStratificationDecorator.Adornment adornment = adornments.get(t);
			...
			if (!headTerm.isGround() && subGoalTerm.isGround()) {
				...
				final IVariable variable = (IVariable) headTerm;
	 ...
	 * @return The new rule.
	 */
	private LocalStratificationDecorator makeNoMatchRule(final LocalStratificationDecorator decorator,
	        final ITuple negatedSubGoalTuple) {
		IRule rule = decorator.getRule();
		final List<LocalStratificationDecorator.Adornment> adornments = decorator.getAdornments();
		final ITuple ruleHead = rule.getHead().iterator().next().getAtom().getTuple();
	...
	public String toString() {
		...
		for (final LocalStratificationDecorator adaptor : mRules) {
	 ...
	 */
	private void adornRules(final Collection<IRule> rules) {
	...
	private LocalStratificationDecorator adornRule(final IRule rule) {
		...
		final List<Adornment> adornments = new ArrayList<Adornment>();

		// TODO For the time being, this is only for rules with a single head
		// predicate
		final Tuple tuple = (Tuple) r.getHead().iterator().next().getAtom().getTuple();
		for (final ITerm term : tuple) {
			...
			if (term.isGround()) {
				...
				final IVariable headVariable = (IVariable) term;
	...
	/**
	 * Helper to scan a rule body looking for built-in predicates that imply that the variable is not equal to some
	 * constant, e.g. ?X < 'a' implies that the rules can not produce any values for ?X equal to 'a'.
	 ...
	 */
	private Adornment extractNotEqualAdornmentsFromRuleBody(final IRule rule, final IVariable headVariable) {
		Adornment adornment = new Adornment();

		// Scan rule body looking for BIN_OP( variable, constant )
		// !=, <, >, not =, not >=, not <=, imply variable != constant adornment
		// If constant is numeric, then can add adornment for not every numeric
		// type
		...
		for (final ILiteral literal : rule.getBody()) {
			...
			if (atom.isBuiltin()) {
				if (positive) {
					// Positive literal
					if ((atom instanceof NotEqualBuiltin) || (atom instanceof NotExactEqualBuiltin)
					        || (atom instanceof LessBuiltin) || (atom instanceof GreaterBuiltin)) {
						not_equal = true;
					}
				} else {
					// Negative literal
					if ((atom instanceof EqualBuiltin) || (atom instanceof ExactEqualBuiltin)
					        || (atom instanceof GreaterEqualBuiltin) || (atom instanceof LessEqualBuiltin)) {
						not_equal = true;
					}
				}
			}
	...
	/**
	 * If we know that a variable can not be equal to a given constant term, then we can add the adornment. However, if
	 * the constant term one of the numeric types, then we can add an adornment for every numeric type.
	 * @param adornment
	 * @param constant
	 * @return
	 */
	private Adornment addNegatedConstant(Adornment adornment, final ITerm constant) {
		if ((constant instanceof IIntegerTerm) || (constant instanceof IFloatTerm) || (constant instanceof IDoubleTerm)
		        || (constant instanceof INumericTerm)) {
			final Number value = (Number) constant.getValue();

...
public class OptimisedProgramStrategyAdaptor implements IEvaluationStrategy {
   ...
   */
  public OptimisedProgramStrategyAdaptor(final IFacts facts, final List<IRule> rules,
  ...
  @Override
  public IRelation evaluateQuery(IQuery query, final List<IVariable> outputVariables) throws EvaluationException {
    if (mMinimalModelComputed)
      ...
      for (final IProgramOptimisation optimisation : mConfiguration.programOptmimisers) {
        final IProgramOptimisation.Result result = optimisation.optimise(rules, query);
      ...
      if (optimised) {
        final IFacts facts = new OriginalFactsPreservingFacts(mFacts, mConfiguration.relationFactory);

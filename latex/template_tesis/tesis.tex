...
\begin{figure}[ht]
    \includegraphics[scale=0.3]{Espacio_busqueda_RepairsFinder.png}
    \centering
...
\begin{theorem}
Dada una \textit{KB} = $(D, \Sigma)$, el algoritmo \textit{RepairsFinder} computa exactamente el conjunto de \textit{reapirs} para esa base de conocimiento.
\end{theorem}
...
toda iteración del ciclo como un conjunto formado por subconjuntos de $k$ elementos de $D$, donde $K$ es un número entre 




\end{proof}
r esta en rep chicos:
notemos que solo se agregan elementos a repairsChicos en la liena 11 de la funcion IterarBottom. notemos que estos elementos son elementos del conjunto Bottom.
notemos que si estamos en la primera iteracion del while en bottom hay solo subconj consistentes (ver inicializacon, en particualr son subconj de un solo elemento).
notemos que si no estamos en la primer iteracion, entonces bottom esta definida en base a la iteracion anterior (linea 14 de iterar bottom). y veamos que en esa misma linea bottom filtra por s consistentes.
...
dado un r cualquiera en el resultado de RepairsFinder quiero ver que r es un repair.

1)
Veamos primero que r es consistente:
Dos opciones: 
r esta en small repairs:
por la inicializacion sabemos que al arrancar el primer ciclo del while en Bottom solo tenemos los subconjuntos de D de un solo elemento.
En la linea 11 de la funcion IterarBottom se agregan a repairsChicos solamente elementos de Bottom (que ya dijimos que son consistentes)
En la linea 14 bottom se vuelve a definir bottom como un conjunto de subconjntos consistentes.
Por lo tanto, dado que bottom siempre se define como una variable que contiene conjuntos consistentes y dado que en small repairs solo agregamos conjuntos de bottom, entonces si r estaba en small repairs, r es consistente.
(notar que en ningun lado del algoritmo se setea algun otro valor a bottom y notar que en ningun lado del algoritmo se agregan cosas a smallRepairs).
...
r esta en repairs grandes:
notemos que en la unica linea donde se agregan elementos a repairsGrandes es en la linea 5 de la funcion AgregarConsistentesEnTopARepairs y que en esa misma linea hay un if que verifica consistencia.


2)
Veamos que es subconjunto de D:
trivialmente: el algoritmo tiene como unica entrada la base de conociiento (D, Sigma) y no hay ninguna otra entada. el algoritmo solo arma subconjuntos con hechos de D y no genera hechos nuevos. por lo tanto r es un subconjunto de D.
...
lema, en culprits solo tengo conjuntos inconsistentes:
demo: solo se agregan en InicializarBottom en la linea 3, con un if que dice claramente que solo se agrega si s es inconsistente. lo mismo para linea 5 de IterarBottom. por lo tanto en culprits solo hay subconj inconsistentes por lo tanto c no es supercnoj de ningun culprit pues c es consistente.

caso 2:
r esta en repairsGrandes.
caso 1: 
r se agergo en la primera iteracion. entonces r es la base de datos-> no existe tal c.
caso 2:
r se agrego en alguna iteracion k con k > 1.
r se agrego a repairsGrandes en la linea 5 de AgregarConsistentesEnTopARepairs,, r es un elemento en top.
top se definio en la iteracion k - 1 como 

...
\SetAlgoLined
$AgregarConsistentesEnTopARepairs()$\;
$topInconsistentes \gets top.filtrarPor(s \rightarrow \neg s.consistente)$\;
...
\SetAlgoLined
 ...
 $proximoBottom \gets subConjuntosDeKElementos(D, nBottom + 1).filtrarPor(s \rightarrow bottom.algunoCumple(b \rightarrow b \subset s$ {\bf and}  $culprits.ningunoCumple(c \rightarrow c \subset s)))$\;
...
\begin{theorem}
Dada una \textit{KB} = $(D, \Sigma)$ inconsistente, el algoritmo \textit{RepairFinder} computa exactamente el conjunto de \textit{reapirs} para esa base de conocimiento.
\end{theorem}
\begin{lemma}

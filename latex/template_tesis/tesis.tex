...
Esto implica que agregar EGDs separables a un conjunto de TGDs no incrementa la complejiad de contestar consultas, tanto en el caso \textit{lineal} como el el \textit{guarded}. Esto es porque alcanza con evaluar si cada EGD se satisface en D, y si esto ocurre podemos entonces armar el chase con las TGDs solamente.
...
Para poder responder las consultas, la implementación de esta \textit{api} debe poder acceder a los otros módulos implementados en el servidor, en los que se encuentra \textit{IRIS} y otro módulos de implementación propia.
...
Una vez obtenido el conjunto de \textit{repairs}, el método \textit{ExecuteAR} procederá de la siguiente manera: para cada \textit{repair} armará un nuevo \textit{programa} en el que los \textit{hechos} serán los mismos \textit{hechos} que se encuentren en ese \textit{repair}. Para cada \textit{programa}, las \textit{consultas} y las \textit{TGDs} serán siempre las mismas, es decir las que el método recibió por parámetro. Una vez mandado a ejecutar todos los programas al módulo de \textit{IRIS}, se computará la intersección de las respuestas para cada consulta, en donde esta intersección consistirá en una intersección de matrices para el caso de las consultas no \textit{booleanas} y en una conjunción lógica en el caso contrario.
